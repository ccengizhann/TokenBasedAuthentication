...
using TokenBasedAuth.Api.Datas.VievModels;
...
namespace TokenBasedAuth.Api.Controllers
{
    ...
    public class AuthenticationController : ControllerBase
    {
        ...
        public AuthenticationController(UserManager<ApplicationUser> userManager, RoleManager<IdentityRole> roleManager, IConfiguration configuration, AppDbContext appDbContext, TokenValidationParameters tokenValidationParameters)
        {
        ...
        // // Handle creation failure with more details for debugging
        ...
        public async Task<IActionResult> LoginAsync([FromBody] LoginVm loginVm)
        {
            ...
            if(userExist is not null && await _userManager.CheckPasswordAsync(userExist, loginVm.Password))
            {
        ...
        private async Task<AuthResultVM> VerifyAndGenerateTokenAsync(TokenRequestVM tokenRequestVM)
        {
            ...
            try
            {
                var tokenCheckResult = jwtTokenHnadler.ValidateToken(tokenRequestVM.Token, _tokenValidationParameters, out var validatedToken);
